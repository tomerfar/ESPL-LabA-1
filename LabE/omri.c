...
// Print symbols of the ELF files
void print_symbols(state* s) {
    // Ensure there are open ELF files to print symbols from
    if (s->fd1 == -1 && s->fd2 == -1) {
        printf("No ELF files opened. Please use 'Examine ELF File' first.\n");
        return;
    }

    void *map_starts[2] = {s->map_start1, s->map_start2};
    int fd[2] = {s->fd1, s->fd2};
    char *file_name[] = {s->file_name1, s->file_name2};

    // Iterate over the open ELF files
    for (int i = 0; i < 2; i++) {

        if (fd[i] == -1)
            continue;;

        Elf32_Ehdr *header = (Elf32_Ehdr *)map_starts[i];
        Elf32_Shdr *section_headers = (Elf32_Shdr *)((char *)map_starts[i] + header->e_shoff);
        const char *shstrtab = (const char *)(map_starts[i] + section_headers[header->e_shstrndx].sh_offset);

        for (size_t j = 0; j < header->e_shnum; j++) {
            if (section_headers[j].sh_type == SHT_SYMTAB) {
                Elf32_Sym *symbols = (Elf32_Sym *)((char *)map_starts[i] + section_headers[j].sh_offset);
                int sym_count = section_headers[j].sh_size / sizeof(Elf32_Sym);
                char *strtab = (char *)map_starts[i] + section_headers[section_headers[j].sh_link].sh_offset;

                // Print debug information if in debug mode
                printf("\n");
                if (s->debug_mode) {
                    printf("Symbol table '%s' contains %d entries:\n", &shstrtab[section_headers[j].sh_name], sym_count);
                }

                printf("File: %s\n", file_name[i]);
                for (int k = 0; k < sym_count; k++) {
                    Elf32_Sym *symbol = &symbols[k];
                    char *section_name = (symbol->st_shndx == SHN_UNDEF) ? "UND" : getSectionName(header, symbol->st_shndx);
...
Elf32_Sym* find_symbol(const char *name, Elf32_Sym *symbols, int symbol_count, const char *strtab) {
...
void check_files_for_merge(state* s) {
    ...
    if (s->fd1 == -1 || s->fd2 == -1) {
        print_error("Two ELF files must be opened for merging.");
        return;
    }

    Elf32_Ehdr *header1 = (Elf32_Ehdr *)s->map_start1;
    Elf32_Ehdr *header2 = (Elf32_Ehdr *)s->map_start2;
    Elf32_Shdr *section_headers1 = (Elf32_Shdr *)(s->map_start1 + header1->e_shoff);
    Elf32_Shdr *section_headers2 = (Elf32_Shdr *)(s->map_start2 + header2->e_shoff);

    // check that each file has exactly one symbol table
    size_t symtabs_counter1 = 0;
    size_t symtabs_counter2 = 0;
    ...
    if (symtabs_counter1 != 1 || symtabs_counter2 != 1) {
        print_error("feature not supported");
        return;
    }

    // get symbol tables' sections
    Elf32_Shdr *symtab_section1 = NULL;
    Elf32_Shdr *symtab_section2 = NULL;
    for (size_t i = 0; i < header1->e_shnum && symtab_section1 == NULL; i++) {
        if (section_headers1[i].sh_type == SHT_SYMTAB) 
            symtab_section1 = &section_headers1[i];
    }
    for (size_t i = 0; i < header2->e_shnum && symtab_section2 == NULL; i++) {
        if (section_headers2[i].sh_type == SHT_SYMTAB) 
            symtab_section2 = &section_headers2[i];
    }

    // get tables, symbol counts and names
    Elf32_Sym *symtab1 = (Elf32_Sym *)((char *)s->map_start1 + symtab_section1->sh_offset);
    Elf32_Sym *symtab2 = (Elf32_Sym *)((char *)s->map_start2 + symtab_section2->sh_offset);
    size_t sym_count1 = symtab_section1->sh_size / sizeof(Elf32_Sym);
    size_t sym_count2 = symtab_section2->sh_size / sizeof(Elf32_Sym);
    char *strtab1 = (char *)(s->map_start1 + section_headers1[symtab_section1->sh_link].sh_offset);
    char *strtab2 = (char *)(s->map_start2 + section_headers2[symtab_section2->sh_link].sh_offset);

    // iterate over all symbols of symtab of first file
    for (size_t i = 1; i < sym_count1; i++) {
        Elf32_Sym *sym = &symtab1[i];
        if (ELF32_ST_BIND(sym->st_info) == STB_GLOBAL) {

            Elf32_Sym *sym2 = find_symbol(&strtab1[sym->st_name], symtab2, sym_count2, strtab2);
    ...
    for (size_t i = 1; i < sym_count2; i++) {
        Elf32_Sym *sym = &symtab2[i];
        if (ELF32_ST_BIND(sym->st_info) == STB_GLOBAL) {

            Elf32_Sym *sym1 = find_symbol(&strtab1[sym->st_name], symtab1, sym_count1, strtab1);
