...
int secondary(int x)
{
    ...
    printf("printing the elements of iarray2:\n");
    for(int i = 0; i < 3; i++){
        printf("value in cell no. %d is %d\n", i, *iarray2Ptr);
        iarray2Ptr++;
    }
    printf("printing the elements of carray2:\n");
    for(int i = 0; i < 3; i++){
        printf("value in cell no. %d is %c\n", i, *carray2Ptr);
        carray2Ptr++;
    }
    int *p;
    printf("value of p %d\n", *p);
...
void point_at(void *p)
{
    ...
    printf("size of long variable in bytes: %ld\n", sizeof(long));
    /*
    long is sufficient for addresses differnces on this 32-bit linux machine, it occupies 4 bytes(32 bits)
    which means it can represent 2^32 different values.
    */
